					...
					/* array in the code)		*/
{
	...
	pd_t *procpd_t=(1024+frame_id)*NBPG;
	unsigned long pdbr=(1024+frame_id)*NBPG;
	//sleep(2);
//	pt_t *pt = 1025*NBPG;
	for(i=0;i<1024;i++){
		if(i<4){
/*			get_frm(&frame_id);
			...
			frm_tab[frame_id].fr_dirty=1;
			frm_tab[frame_id].fr_refcnt=4;
*/			procpd_t[i].pd_base=1025+i;//1024+frame_id; //changed it
//			pt=(1024+frame_id)*NBPG;
			procpd_t[i].pd_pres=1;
			procpd_t[i].pd_write=1;
			procpd_t[i].pd_user=0;
			procpd_t[i].pd_pwt=0;
			procpd_t[i].pd_pcd=0;
			procpd_t[i].pd_acc=0;
			procpd_t[i].pd_mbz=0;
			procpd_t[i].pd_fmb=0;
			procpd_t[i].pd_global=0;
			procpd_t[i].pd_avail=0;
/*			for(j=0;j<1024;j++){
...
*/		}
			...
			procpd_t[i].pd_base=0;
			procpd_t[i].pd_pres=0;
			procpd_t[i].pd_write=1;
			procpd_t[i].pd_user=0;
			procpd_t[i].pd_pwt=0;
			procpd_t[i].pd_pcd=0;
