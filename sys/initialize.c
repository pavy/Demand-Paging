...
int page_replace_policy = SC;//AGING;//SC;
...
*/
	...
	int nullframe;
	get_frm(&nullframe);
	//frm_tab[nullframe].fr_vpno=-1;
	//frm_tab[i].fr_refcnt
	frm_tab[nullframe].fr_type=FR_DIR;
	frm_tab[nullframe].fr_dirty=1;
	pd_t *nullpd_t=1024*NBPG;
	pt_t *pt = 1025*NBPG;
	...
	for(i=0;i<1024;i++){
		if(i<4){
			...
			//frm_tab[nullframe].fr_vpno=-1;
			//frm_tab[i].fr_refcnt
			frm_tab[nullframe].fr_type=FR_TBL;
			frm_tab[nullframe].fr_dirty=1;
			frm_tab[nullframe].fr_refcnt=4;
			nullpd_t[i].pd_base=1024+nullframe; //changed it
			nullpd_t[i].pd_pres=1;
			nullpd_t[i].pd_write=1;
			nullpd_t[i].pd_user=0;
			nullpd_t[i].pd_pwt=0;
			nullpd_t[i].pd_pcd=0;
			nullpd_t[i].pd_acc=0;
			nullpd_t[i].pd_mbz=0;
			nullpd_t[i].pd_fmb=0;
			nullpd_t[i].pd_global=0;
			nullpd_t[i].pd_avail=0;
			for(j=0;j<1024;j++){
				pt[(i*1024)+j].pt_base=(i*1024)+j;
				pt[(i*1024)+j].pt_pres=1;
				pt[(i*1024)+j].pt_write=1;
				pt[(i*1024)+j].pt_user=0;
				pt[(i*1024)+j].pt_pwt=0;
				pt[(i*1024)+j].pt_pcd=0;
				pt[(i*1024)+j].pt_acc=0;
				pt[(i*1024)+j].pt_dirty=0;
				pt[(i*1024)+j].pt_mbz=0;
				pt[(i*1024)+j].pt_global=0;
				pt[(i*1024)+j].pt_avail=0;
			}
		}
		else{
			nullpd_t[i].pd_base=0; //changed it
			nullpd_t[i].pd_pres=0;
			nullpd_t[i].pd_write=1;
			nullpd_t[i].pd_user=0;
			nullpd_t[i].pd_pwt=0;
			nullpd_t[i].pd_pcd=0;
